  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full text-[#2C2926]">
        ...
        <div className="container mx-auto px-10 max-w-[1600px] py-16 flex flex-col lg:flex-row gap-20">
          <aside className="lg:w-64 shrink-0 relative">
            <div className="sticky top-32 space-y-8 pr-10 border-r border-white/10 min-h-[400px] flex flex-col justify-between">
              ...
              {/* 💡 Desktop Back to Top (Hidden on Mobile) */}
              <AnimatePresence>
                {showScrollTop && (
                  <motion.button
                    initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
                    onClick={scrollToTop}
                    className="hidden lg:flex mt-10 self-start ml-5 w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/40 text-gray-400 hover:text-[#2C2926] hover:bg-white/40 transition-all items-center justify-center shadow-lg group active:scale-90"
                  >
        ...
        {/* 💡 Mobile Sticky Back to Top Button (Hidden on Desktop) */}
        <AnimatePresence>
          {showScrollTop && (
            <motion.button
              ...
              onClick={scrollToTop}
              // 💡 bottom-10 left-6 ပြောင်းလိုက်လို့ Chat bot နဲ့ မကွယ်တော့ပါဘူး
              className="lg:hidden fixed bottom-10 left-6 z-50 w-12 h-12 rounded-full bg-[#2C2926]/80 backdrop-blur-md text-white shadow-2xl flex items-center justify-center active:scale-90 transition-transform border border-white/20"
            >
